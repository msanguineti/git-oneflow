...
// const composeBranchName = async (cmd: commander.Command, name: string) => {
...
const cmdAction = async (arg: unknown, cmd: unknown): Promise<void> => {
  const name =
    arg ??
    (
      await prompt.promptUser([
        prompt.askInput({
          message: `${
            (cmd as commander.Command)._name.charAt(0).toUpperCase() +
            (cmd as commander.Command)._name.slice(1)
          } name?`,
          name: 'branchName',
          validate: (input) =>
  ...
  const base =
    (cmd as commander.Command).base ??
    config.getBaseBranch((cmd as commander.Command)._name)

  const ref =
    (cmd as commander.Command).ref ??
    getDefaultReference((cmd as commander.Command)._name)

  git.createBranch(base ? `${base}/${name}` : name, ref)
}

const extraOptions: gofCommand.GofCmdOption[] = [
  {
    flags: '-r, --ref <ref>',
